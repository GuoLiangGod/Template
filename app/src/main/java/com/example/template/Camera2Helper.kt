...
class Camera2Helper(
    ...
    private val mTextureView2: TextureView
) {
...
//            mActivity.toast("相机硬件不支持新特性")
        ...
        mTextureView2.surfaceTexture?.setDefaultBufferSize(mPreviewSize.width, mPreviewSize.height)
    ...
    private fun createCaptureSession(cameraDevice: CameraDevice) {
        ...
        val surface2 = Surface(mTextureView2.surfaceTexture)
        captureRequestBuilder.addTarget(surface)  // 将CaptureRequest的构建器与Surface对象绑定在一起
        captureRequestBuilder.addTarget(surface2)  // 将CaptureRequest的构建器与Surface对象绑定在一起
        ...
        cameraDevice.createCaptureSession(
            arrayListOf(surface, mImageReader?.surface, surface2),
